...
class TestDram(unittest.TestCase):
    def setUp(self):
        ...
        cocotb_compile_and_run([
            ...
            os.path.join(fw_path, 'ddr_fifo/ddr_fifo_core.v'),
            os.path.join(fw_path, 'ddr_fifo/ddr_fifo.v'),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), '../firmware/src/dram_test.v'),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), '../tests/tb.v')],
            top_level='tb',
            sim_bus='basil.utils.sim.SiLibUsbBusDriver'
        )

        with open(os.path.join(os.path.dirname(__file__), '../dram_test.yaml'), 'r') as f:
            cnfg = yaml.load(f)
